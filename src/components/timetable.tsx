...
export function Timetable() {
    ...
    const getClass = (day: string, time: string) => {
        return sessions.find(
            (session) =>
                session.week_day === day && timeSlotMatches(session.starting_time, session.ending_time, time),
        );
    };
    ...
    const ClassBadge = ({ type }: { type: string }) => {
        ...
        return <span className={`text-xs px-2 py-1 rounded-full ${bgColorClass}  text-white`}>{type}</span>;
    };
    ...
    const ClassCell = ({ classData }: { classData?: Ursession }) => {
        ...
        return (
            <div
                className={`p-2 ${bgColorClass} rounded-md h-full flex flex-col hover:opacity-80 translate-x-1 transition-transform`}
            >
                <div className="font-medium text-sm">{classData.module}</div>
                {classData.room && <div className="text-xs text-gray-600">Room {classData.room}</div>}
